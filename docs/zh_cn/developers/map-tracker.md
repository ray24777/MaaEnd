...
- `rotation_lower_threshold`: 介于 $(0, 180]$ 的实数，默认 `6.0`。判断需要微调朝向的方向角偏离阈值，单位是度。

- `rotation_upper_threshold`: 介于 $(0, 180]$ 的实数，默认 `30.0`。判断需要大幅调整朝向的方向角偏离阈值，单位是度。此时玩家将会使用更慢的速度转向。
